...
2. Use the latest complete fourteen-day window. Keep an issue stable within its biweekly window; preserve each prior Nepal issue as structured history when advancing. Record windowStart/windowEnd and explicit event dates.
...
Run `npm run build`, `npm audit --omit=dev`, and UI tests. Check actual counts, current-window dates, translations, official legal domains, links, mobile widths and sealed-history isolation. Generated data is rebuilt, not hand-edited. Public output and Git history must not contain credentials or former-market documents.

Commit intentional files and push only Adam123wu/nepal-ict-briefing main under the user's publication authorization. Wait for Pages and check all four public routes in both languages at https://adam123wu.github.io/nepal-ict-briefing/; verify `/archive/` and `/compliance/` return 404. Do not publish to a different repository.
...
Tracked topics run daily at 07:00 Asia/Kathmandu after collection and editorial checks. `config/tracked-topics.json` holds repository-defined topics; open issues labeled `tracking-request` may add current report item IDs. The monitor compares current reviewed copy and matching collected candidates with `config/tracking-state.json`. It sends either a change notice or an explicit no-update email through Gmail SMTP secrets. A candidate match is a lead for review, not a fact eligible for publication.
...
Daily collection is scheduled at 00:00 Asia/Baghdad (02:45 Nepal); editorial review at 00:20 Baghdad (03:05 Nepal); cloud watchdog at 02:30 Baghdad (05:15 Nepal). Inspect workflow records for missed or failed runs. The visible search timestamp or fallback title list is not an editorial publication timestamp. Run `python3 scripts/preserve_nepal_history.py` after each completed 14-day window; `config/history-reports.json` must be committed and existing completed issues must never be overwritten. The build expands the current and every completed issue inside the unified Briefing page; no standalone Archive route or issue-tab click is used. Advance the next issue by 14 days only after preserving the previous issue; do not relabel old news or invent empty completed issues. Keep the previous reviewed issue visible if new research is incomplete, explicitly noting the lag. The daily cloud collector also preserves completed history independently of local Codex availability.
