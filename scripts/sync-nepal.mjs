...
export async function syncNepal(){
 ...
 await save('data/nepal.json',{market,report,sources,people,compliance,legal,signals:social,telegram:feed,archive,focus:focus.focusAreas.map(({id,label,labelEn,priority})=>({id,label,labelEn,priority}))});
 const esc=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
 const html=`<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>尼泊尔 ICT 双周简报</title><style>body{font:16px/1.8 system-ui;background:#f5f7fb;color:#172033;max-width:1000px;margin:40px auto;padding:24px}article,section{background:white;padding:24px;margin:16px 0;border-radius:16px}a{color:#1459aa}</style><h1>🇳🇵 尼泊尔 ICT 双周简报 · ${esc(report.issue)}</h1><p>${esc(report.period)}</p><p>${esc(report.status)}</p>${report.countries.np.sections.map(s=>`<section><h2>${esc(s.category)}</h2>${s.items.length?s.items.map(i=>`<article><h3>${esc(i.title)}</h3><p>${esc(i.date)} · ${esc(i.badge)}</p><p>${esc(i.text)}</p><p>${esc(i.opportunity||"")}</p>${i.links.map(l=>`<a href="${esc(l.url)}">${esc(l.label)}</a>`).join(' ')}</article>`).join(''):'<p>本栏目尚无完成核验的尼泊尔新闻。</p>'}</section>`).join('')}</html>`;
