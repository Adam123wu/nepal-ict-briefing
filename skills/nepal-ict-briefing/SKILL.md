...
9. On every successful daily review, even when no new event qualifies, update `config/editorial-status.json` with the Asia/Baghdad review date, UTC review timestamp, reviewed commit and candidate timestamp. Clear `config/deepseek-fallback-digest.json` after reviewing any fallback candidates. A site build or candidate collection is not a completion marker.
...
GitHub Actions collects public candidates daily at 02:45 Nepal time. The existing Codex editorial task runs daily at 00:20 Asia/Baghdad / 03:05 Nepal and performs research, translation, review and publication using this skill. Local-file scheduled work needs the computer and app running with required access. At 02:30 Baghdad / 05:15 Nepal, `editorial-watchdog.yml` checks the dated Codex completion marker. If it is missing, and same-day T1/T2 website candidates exist, `scripts/fallback_nepal_deepseek.py` makes one bounded DeepSeek request for title translation and section classification. Its visible output is explicitly unverified and excluded from formal news/opportunity counts until Codex reads the original sources. Stale collection, missing credentials, invalid model output or API failure must fail closed and leave the formal report unchanged. Report missed/failed research honestly; do not equate collection or fallback triage with an updated briefing. Do not silently add paid APIs or send alerts to new recipients.

## Midnight collection and biweekly sealing

Daily collection is scheduled at 00:00 Asia/Baghdad (02:45 Nepal); editorial review at 00:20 Baghdad (03:05 Nepal); cloud watchdog at 02:30 Baghdad (05:15 Nepal). Inspect workflow records for missed or failed runs. The visible search timestamp and fallback title list are not editorial publication timestamps. Run `python3 scripts/preserve_nepal_history.py` after each completed 14-day window; `config/history-reports.json` must be committed and existing completed issues must never be overwritten. The build must expand every completed issue inside the Briefing page rather than publish a separate Archive route. Advance the next issue by 14 days only after preserving the previous issue; do not relabel old news or invent empty completed issues. Keep the previous reviewed issue visible if new research is incomplete, explicitly noting the lag. The daily cloud collector also preserves completed history independently of local Codex availability.
