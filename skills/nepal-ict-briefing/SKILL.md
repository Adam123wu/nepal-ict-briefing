...
Publish Nepal-only current and completed issues inside the single Briefing interface. Sealed bilingual history remains immutable input, but the standalone Archive page and navigation stay removed. Do not restore historical Iraq, Jordan or Lebanon reports, people or feeds in this repository or site. This does not authorize deleting separate legacy repositories or rewriting Git history. Keep Compliance & Law absent from desktop/mobile navigation and direct public routes; test that `/archive/` and `/compliance/` return 404.
